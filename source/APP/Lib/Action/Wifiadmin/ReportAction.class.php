...
class ReportAction extends AdminAction{
	...
	private  function getuserchart(){
    	...
    	switch(strtolower($way)){
    		case "today":
				...
				$sql.=" where add_date='".date("Y-m-d")."' and ( mode=0 or mode=1 or mode=3 ) ";
    		...
    		case "yestoday":
				...
				$sql.=" where add_date=DATE_ADD(CURDATE() ,INTERVAL -1 DAY) and ( mode=0 or mode=1 or mode=3 )  ";
				$sql.=" )a group by thour ) c ";
				$sql.="  on a.t=c.thour ";
    		...
    		case "week":
				...
				$sql.=" where  add_date between DATE_ADD(CURDATE() ,INTERVAL -6 DAY) and CURDATE()  and ( mode=0 or mode=1 or mode=3 ) GROUP BY  add_date";
    		...
    		case "month":
				...
				$sql.=" where    add_date >= '".date("Y-m-01")."' and ( mode=0 or mode=1 or mode=3 ) GROUP BY  add_date";
    		...
    		case "query":
				...
				$sql.="( select add_date,count(id) as totalcount , count(CASE when mode=0 then 1 else null end) as regcount, count(CASE when mode=1 then 1 else null end) as phonecount, count(CASE when mode=3 then 1 else null end) as weixincount  from ".C('DB_PREFIX')."member ";
				$sql.=" where  add_date between '$sdate' and '$edate'  and ( mode=0 or mode=1 or mode=3 ) GROUP BY  add_date";
				$sql.=" ) b on a.td=b.add_date ";
	...
	private function getauthrpt(){
    	...
    	switch(strtolower($way)){
    		case "today":
    			$sql=" select t,CONCAT(CURDATE(),' ',t,'点') as showdate, COALESCE(ct,0)  as ct ,COALESCE(ct_reg,0)  as ct_reg,COALESCE(ct_phone,0)  as ct_phone,COALESCE(ct_key,0)  as ct_key, COALESCE(ct_weixin,0)  as ct_weixin from ".C('DB_PREFIX')."hours a left JOIN ";
				$sql.="( select thour ,count(case when mode=0 or mode=1 or mode=2 or mode=3 then 1 else null end) as ct ,count(case when mode=0 then 1 else null end) as ct_reg,count(case when mode=1 then 1 else null end) as ct_phone,count(case when mode=2 then 1 else null end) as ct_key,count(case when mode=3 then 1 else null end) as ct_weixin from ";
    			...
    			break;
    		case "yestoday":
    			$sql=" select t, CONCAT(CURDATE()-1,' ',t,'点') as showdate,COALESCE(ct,0)  as ct,COALESCE(ct_reg,0)  as ct_reg,COALESCE(ct_phone,0)  as ct_phone,COALESCE(ct_key,0)  as ct_key, COALESCE(ct_weixin,0)  as ct_weixin from ".C('DB_PREFIX')."hours a left JOIN ";
				$sql.="( select thour ,count(case when mode=0 or mode=1 or mode=2 or mode=3 then 1 else null end) as ct ,count(case when mode=0 then 1 else null end) as ct_reg,count(case when mode=1 then 1 else null end) as ct_phone,count(case when mode=2 then 1 else null end) as ct_key,count(case when mode=3 then 1 else null end) as ct_weixin from ";
				$sql.="(select shopid,mode,FROM_UNIXTIME(login_time,\"%H\") as thour,";
				$sql.=" FROM_UNIXTIME(login_time,\"%Y-%m-%d\") as d from ".C('DB_PREFIX')."authlist ) a ";
    			...
    			break;
    		case "week":
    			$sql="  select td as showdate,right(td,5) as td,datediff(td,CURDATE()) as t,COALESCE(ct,0)  as ct ,COALESCE(ct_reg,0)  as ct_reg,COALESCE(ct_phone,0)  as ct_phone,COALESCE(ct_key,0)  as ct_key, COALESCE(ct_weixin,0)  as ct_weixin from ";
    			$sql.=" ( select CURDATE() as td ";
    			for($i=1;$i<7;$i++){
    				$sql.="  UNION all select DATE_ADD(CURDATE() ,INTERVAL -$i DAY) ";
    			}
    			$sql.=" ORDER BY td ) a left join ";
    			$sql.="( select add_date,count(case when mode=0 or mode=1 or mode=2 or mode=3 then 1 else null end) as ct ,count(case when mode=0 then 1 else null end) as ct_reg,count(case when mode=1 then 1 else null end) as ct_phone,count(case when mode=2 then 1 else null end) as ct_key,count(case when mode=3 then 1 else null end) as ct_weixin from ".C('DB_PREFIX')."authlist ";
    		...
    		case "month":
    			$t=date("t");
    			$sql=" select tname as showdate,tname as t, COALESCE(ct,0) as ct ,COALESCE(ct_reg,0)  as ct_reg,COALESCE(ct_phone,0)  as ct_phone,COALESCE(ct_key,0)  as ct_key, COALESCE(ct_weixin,0)  as ct_weixin from ".C('DB_PREFIX')."day  a left JOIN";
				$sql.="( select right(add_date,2) as td ,count(case when mode=0 or mode=1 or mode=2 or mode=3 then 1 else null end) as ct,count(case when mode=0 then 1 else null end) as ct_reg,count(case when mode=1 then 1 else null end) as ct_phone,count(case when mode=2 then 1 else null end) as ct_key,count(case when mode=3 then 1 else null end) as ct_weixin  from ".C('DB_PREFIX')."authlist  ";
