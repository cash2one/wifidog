...
protected function _initialize(){
       ...
       include CONF_PATH.'enum/enum.php';//$enumdata
		...
		if(IS_POST){
			...
			$_POST['trade']=$lv;
				...
				if($_POST['showcount']=="ok"){$_POST['showcount']=1;}
				else{$_POST['showcount']=0;}
	...
	public function edit(){
		...
		//array_push($enumdata['trades'],array('key'=>'全行业','txt'=>'全行业'));
		...
		if(IS_POST){
            ...
            if($result)
            {
                ...
                $_POST['startdate']=strtotime($_POST['startdate']);
	            $_POST['enddate']=strtotime($_POST['enddate']);
	           if($_POST['state']=="ok"){$_POST['state']=1;}
				else{$_POST['state']=0;}
				if($_POST['showcount']=="ok"){$_POST['showcount']=1;}
				else{$_POST['showcount']=0;}
                if($db->create()){
		...
		$aid =I('post.id');
	    	...
	    	switch(strtolower($way)){
	    		case "today":
					...
					$sql.=" where add_date='".date("Y-m-d")."' and (mode=50 or mode=99) and aid=".$aid;
					$sql.=" )a group by thour ) c ";
					$sql.="  on a.t=c.thour ";
	    		...
	    		case "yestoday":
					...
					$sql.=" where add_date=DATE_ADD(CURDATE() ,INTERVAL -1 DAY) and (mode=50 or mode=99) and aid=".$aid;
					$sql.=" )a group by thour ) c ";
					$sql.="  on a.t=c.thour ";
	    		...
	    		case "week":
	    			...
	    			$sql.=" ORDER BY td ) a left join ";
	    			$sql.="( select add_date,sum(showup) as showup ,sum(hit) as hit from ".C('DB_PREFIX')."adcount";
					$sql.=" where   add_date between DATE_ADD(CURDATE() ,INTERVAL -6 DAY) and CURDATE() and (mode=50 or mode=99) and aid=".$aid."  GROUP BY  add_date";
					$sql.=" ) b on a.td=b.add_date ";
	    		...
	    		case "month":
	    			...
	    			$sql=" select tname as showdate,tname as t, COALESCE(showup,0)  as showup, COALESCE(hit,0)  as hit,COALESCE(hit/showup*100,0) as rt from ".C('DB_PREFIX')."day  a left JOIN";
					$sql.="( select right(add_date,2) as td ,sum(showup) as showup ,sum(hit) as hit  from ".C('DB_PREFIX')."adcount  ";
					$sql.=" where   add_date >= '".date("Y-m-01")."' and (mode=50 or mode=99) and aid=".$aid." GROUP BY  add_date";
					$sql.=" ) b on a.tname=b.td ";
					$sql.=" where a.id between 1 and  $t";
	    		...
	    		case "query":
					...
					$sql.="( select add_date,sum(showup) as showup ,sum(hit) as hit  from ".C('DB_PREFIX')."adcount ";
					$sql.=" where  add_date between '$sdate' and '$edate'  and (mode=50 or mode=99) and aid=".$aid ." GROUP BY  add_date";
					$sql.=" ) b on a.td=b.add_date ";
