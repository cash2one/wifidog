...
class IndexAction extends  BaseAction{
	...
	public  function getuserchart(){
    	...
    	switch(strtolower($way)){
    		case "today":
				...
				$sql.=" and tt.add_date='".date("Y-m-d")."' and ( tt.mode=0 or tt.mode=1 or tt.mode=3) ";
   ...
   public function getauthrpt(){
    ...
    $where['id']=$id;
     ...
     $r = D('Shop')->where($where)->find();
     
     if($r==false){
        $this->error('没有此用户信息');
        }else{
          if(D('Shop')->where($where)->delete()){
			  $Dao = M("Routemap");
              $where1['shopid']=$id;
              $result = $Dao->where($where1)->delete(); 
          	$this->success('删除成功');
          }else{
          	$this->error('删除失败');
          }
    ...
    private function getroutechart(){
		...
		$sql.=" where shopid in (select id from ".C('DB_PREFIX')."shop where pid=".$this->aid.")";
    	$db=D("Routemap");
    	$info=$db->query($sql);
	...
	private function getroutelist(){
    	...
    	$sql.=$where;
		$sql.=" and shopid in (select id from ".C('DB_PREFIX')."shop where pid=".$this->aid.")";
	    	
    	$sqlcount=" select count(*) as ct from ".C('DB_PREFIX')."routemap ";
    	$sqlcount.=$where;
		$sqlcount.=" and shopid in (select id from ".C('DB_PREFIX')."shop where pid=".$this->aid.")";
    	$rs=$db->query($sqlcount);
    	$count=$rs[0]['ct'];
    	$page=new AdminAjaxPage($count,4);
	...
	public function dopwdsh(){
		...
		if(IS_POST){
            ...
            $_POST['password']=md5($pwd);
            $where['id']=$id;
			$resu=D('shop')->where($where)->field('password')->find();
			if ($resu['password']==$_POST['password']){
				$data['error']=1;
		    		$data['msg']="请输入与原密码不一样的密码";
		    		return $this->ajaxReturn($data);
			}
         if(D('shop')->where($where)->save($_POST)){
				$data['error']=0;
	    		$data['msg']="更新成功";
	    		return $this->ajaxReturn($data);
			}else{
