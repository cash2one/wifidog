	 ...
	 function exportexcel($data=array(),$title=array(),$filename='report'){
	    ...
	    if (!empty($data)){
	        foreach($data as $key=>$val){
	            ...
	            $data[$key]=implode("</td><td style=text-align:center>", $data[$key]);
	            echo "<tr><td style=text-align:center>$data[$key]</td></tr>\n";
	        }
	        //echo implode("<tr><td>\n",$data);
			echo "</table>";
	    }
	  ...
	  */
	  function exportexcelByKey($data=array(),$title=array(),$filename='report'){
	    ...
	    if (!empty($title)){
			...
			echo "<table border=1>";
	        $excelhead= implode("</th><th>", $excelhead);
	        
	        echo "<tr><th>$excelhead</th></tr>\n";
	    }
	    ...
	    if (!empty($data)){
	        foreach($data as $obj){
	            	...
	            	$Line= implode("</td><td style=text-align:center>", $Line);
	            	echo "<tr><td style=text-align:center>$Line</td></tr>\n";
	...
	function showclass_common($vo,$cid){
		switch (strtolower($vo['mode'])){
			...
			case "3":
				...
				//坐标是纬度进度
				$url="http://api.map.baidu.com/marker?location=".$vo['point_y'].",".$vo['point_x']."&title=".$vo['info']."&content=".$vo['info']."&output=html&src=weiyibai|weiyibai";
				return $url;
